...
describe('剪贴板分享链接识别', () => {
  ...
  it('识别旧域名 aliyundrive.com 的分享链接', () => {
    expect(detectShareLink('https://www.aliyundrive.com/s/abc123')?.url).toBe('https://www.aliyundrive.com/s/abc123')
  })

  it('不识别其他网盘链接', () => {
    expect(detectShareLink('https://pan.baidu.com/s/1abc_DEF?pwd=2x3y')).toBeUndefined()
    expect(detectShareLink('pan.quark.cn/s/abc123')).toBeUndefined()
  })
