...
describe('book external metadata', () => {
  it('uses Google Books title and author search without downloading the book file', async () => {
    ...
    expect(String(requestMock.mock.calls[0][0])).toContain('www.googleapis.com/books/v1/volumes')
    expect(String(requestMock.mock.calls[0][0])).toContain('intitle%3A%E4%B8%89%E4%BD%93')
    expect(String(requestMock.mock.calls[0][0])).toContain('inauthor%3A%E5%88%98%E6%85%88%E6%AC%A3')
  })

  it('calls Google Books only once for a lookup', async () => {
    const requestMock = vi.fn(async () => ({ ok: true, json: async () => ({ items: [{ volumeInfo: { title: 'The Wailing Wind', authors: ['Tony Hillerman'], imageLinks: { thumbnail: 'http://books.google.test/cover.jpg' } } }] }) }))
    const logs: string[] = []
