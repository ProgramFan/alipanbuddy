...
use base64::Engine;
use boxcore::flowenc::Alg;
use boxcore::upload::{UploadEncryption, UploadPartRequest, UploadPartResult};
...
pub async fn file_prehash(path: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || boxcore::hashing::prehash(&PathBuf::from(path)).map_err(|e| e.to_string()))
        .await
...
pub async fn file_sha1(window: Window, app: AppHandle, task_id: u64, path: String, access_token: String) -> Result<Sha1Result, String> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        ...
        let hash = boxcore::hashing::sha1_file(&path, &cancel_for_task, |read, size| {
        ...
        let proof_code = boxcore::hashing::proof_code(&path, &access_token, size).map_err(|e| e.to_string())?;
...
pub async fn upload_part(window: Window, app: AppHandle, task_id: u64, path: String, start: u64, size: u64, url: String, authorization: String, encryption: Option<EncryptionArg>) -> Result<UploadPartResult, String> {
    ...
    let result = boxcore::upload::upload_part(&client, UploadPartRequest { path: PathBuf::from(path), start, size, url, authorization, encryption }, cancel, limiter, progress).await;
...
pub async fn flowenc_file(alg: String, password: String, src: String, dst: String) -> Result<u64, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || boxcore::encfile::transform_file(alg, &password, &PathBuf::from(src), &PathBuf::from(dst)).map_err(|e| e.to_string()))
        .await
        .map_err(|e| e.to_string())?
}
...
pub fn flowenc_bytes(alg: String, password: String, size_salt: u64, base64: String) -> Result<String, String> {
    ...
    boxcore::encfile::transform_bytes(alg, &password, size_salt, &mut data).map_err(|e| e.to_string())?;
